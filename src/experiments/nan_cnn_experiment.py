...
def train_and_eval_weight_share_nan_cnn(  
                    train_data, 
                    test_data, 
                    seed_worker,
                    input_dims,
    ...
    for epoch in range(n_epochs):
        ...
        print(f"Epoch [{epoch + 1}/{n_epochs}], Task Training Loss: {avg_classifier_loss:.4f}, Accuracy: {classification_accuracy:.2f}, Task Test Loss: {avg_test_loss}, Task Test Accuracy: {test_accuracy}%")
...
def train_annealing_linear_weight_share_nan_cnn(  
                    train_data, 
                    test_data,
                    input_dims,
                    seed_worker,
    ...
    for epoch in range(n_classifier_epochs):
        ...
        print(f"Epoch [{epoch + 1}/{n_classifier_epochs}], Task Training Loss: {avg_classifier_loss:.4f}, Accuracy: {classification_accuracy:.2f}%, Task Test Loss: {avg_test_loss}, Task Test Accuracy: {test_accuracy}% LR: {current_lr:.6f}")
        """print(
                f"epoch {epoch+1}: "
                f"allocated={torch.cuda.memory_allocated()/1024**2:.1f} MB, "
                f"reserved={torch.cuda.memory_reserved()/1024**2:.1f} MB, "
...
""" the remainder of training functions are experimental only and were not discussed within the report..."""
...
def train_nan_cnn_diverse_activations_show_features_localised(  data, 
    ...
    for epoch in range(n_epochs):
        ...
        for images, labels in train_loader:
          ...
          #since every filter compares itself to n_filters-1 others
