...
""" linear schedule for the neuron based autoencoder network... training the hidden layer to 5000 iterations, then the output to 5000
"""
def linear_schedule_nan_hill_climb(model, data_loader,criterion, device, rng, epoch):
    ...
    else:  #(when epoch > 5000) set indexes for output layer parameter perturbation
...
""" Both hidden layer and output layer receive parameter perturbations every iteration
 """
def parallel_step_schedule_hill_climb(model, data_loader,criterion, device, rng):
    ...
    #now also perturb the output layer ... 
...
def get_random_hidden_param(model, selected_neuron, rng,): 
    ...
    #quantify the possible params for every node in the hidden layer
    params_per_neuron = (
