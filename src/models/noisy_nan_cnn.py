...
class SimpleWeightShareConvFilter(nn.Module):
    ...
    def __init__(self, kernel_size=3,stride=1,padding=1,output_padding=0,bias=False,sigmoid=True,latent_sigma=0.2):
...
class FilterCNN(nn.Module):
    ...
    def __init__(
        ...
        self.filters = nn.ModuleList([SimpleWeightShareConvFilter(kernel_size=kernel_size,stride=stride,padding=padding,output_padding=output_padding,bias=bias,sigmoid=sigmoid,latent_sigma=latent_sigma)for _ in range(n_filters)])
