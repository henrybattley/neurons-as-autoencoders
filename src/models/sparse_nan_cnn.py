...
class SimpleWeightShareConvFilter(nn.Module):
    ...
    def sparse_encode(self, x):
        ...
        sparsity_loss = self.lambda_sparse * h.abs().mean()


        return h,sparsity_loss
    
    #calls encode and decode the latent feature representation (used by individual filters)
    
    def forward(self, x):

        h,sparsity_loss = self.sparse_encode(x)
        ...
        return torch.sigmoid(x_hat),sparsity_loss
...
class FilterCNN(nn.Module):
    ...
    def __init__(
        ...
        self.filters = nn.ModuleList([SimpleWeightShareConvFilter(kernel_size=kernel_size,stride=stride,padding=padding,output_padding=output_padding,bias=bias,sigmoid=sigmoid,lambda_sparse=lambda_sparse)for _ in range(n_filters)])
